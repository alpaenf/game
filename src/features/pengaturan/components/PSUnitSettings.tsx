...
export default function PSUnitSettings() {
  ...
  if (loading) {
    return <div className="p-6 text-center text-sm text-gray-400">Loading data mesin...</div>;
  }
    ...
    <>
      <div className="p-6">
        ...
        <div className="space-y-3 mb-6">
          {units.map((unit) => (
            <div key={unit.id} className="flex justify-between items-center bg-background p-4 rounded-xl border border-gray-800">
              <div>
                ...
                {/* <p className="text-xs text-gray-500">{unit.status === 'in_use' ? 'Sedang Dipakai' : 'Idle'}</p> */}
              </div>
              <div className="flex items-center gap-4">
                <div className="text-gray-300 font-mono text-sm">{formatRupiah(unit.hourly_rate)} / Jam</div>
                <div className="flex gap-2">
                   <button onClick={() => openForm(unit)} className="p-1.5 text-gray-400 hover:text-white bg-gray-800 hover:bg-gray-700 rounded transition-colors">
        ...
        <button 
          onClick={() => openForm()}
          className="w-full py-2.5 bg-gray-800 text-white font-medium rounded-lg hover:bg-gray-700 transition-colors border border-gray-700 flex items-center justify-center gap-2"
        >
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-surface border border-gray-800 p-6 rounded-2xl w-full max-w-sm shadow-xl">
             ...
             <form onSubmit={handleSave} className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Nama Mesin / TV</label>
                  <input
                    type="text"
                    required
                    value={name}
                    ...
                    className="w-full bg-background border border-gray-700 rounded-lg p-2.5 text-sm text-white focus:border-primary outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Tipe Konsol</label>
                  <select
                    value={type}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Tarif per Jam (Rp)</label>
                  <input
                    type="number"
                    required
                    min="0"
