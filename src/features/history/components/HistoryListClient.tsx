  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Riwayat & Laporan</h1>
          <p className="text-gray-400">Pantau transaksi dan rekap pendapatan harian outlet.</p>
        </div>

        {/* Filter Tanggal */}
        <div className="flex items-center gap-2 bg-surface p-2 rounded-xl border border-gray-800">
           <Calendar className="w-5 h-5 text-gray-400 ml-2" />
           <select 
             value={dateFilter} 
             onChange={(e) => setDateFilter(e.target.value)}
             className="bg-transparent text-white border-none focus:ring-0 outline-none pr-4 py-1"
           >
           ...
           <input 
             ...
             onChange={(e) => setDateFilter(e.target.value)}
             className="bg-transparent text-gray-400 border border-gray-700 rounded p-1 text-sm outline-none focus:border-primary w-8 h-8 cursor-pointer"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-surface p-5 rounded-2xl border border-gray-800 flex items-center gap-4">
            <div className="p-3 bg-green-500/10 rounded-xl text-green-500">
                <TrendingUp className="w-6 h-6" />
            </div>
            <div>
                <p className="text-sm text-gray-400 font-medium">Total Omset</p>
                <h3 className="text-2xl font-bold text-white">{formatRupiah(summary.totalOmset)}</h3>
            </div>
        </div>
        <div className="bg-surface p-5 rounded-2xl border border-gray-800 flex items-center gap-4">
            ...
            <div>
                <p className="text-sm text-gray-400 font-medium">Biaya Rental PS</p>
                <h3 className="text-xl font-bold text-white">{formatRupiah(summary.totalPSTotal)}</h3>
            </div>
        </div>
        <div className="bg-surface p-5 rounded-2xl border border-gray-800 flex items-center gap-4">
            ...
            <div>
                <p className="text-sm text-gray-400 font-medium">Snack & Makanan</p>
                <h3 className="text-xl font-bold text-white">{formatRupiah(summary.totalItemsTotal)}</h3>
            </div>
        </div>
        <div className="bg-surface p-5 rounded-2xl border border-gray-800 flex items-center gap-4">
           <div className="p-3 bg-primary/10 rounded-xl text-primary">
            ...
            <div>
                <p className="text-sm text-gray-400 font-medium">Transaksi Lunas</p>
                <h3 className="text-2xl font-bold text-white">{summary.totalTransactions} / Sesi</h3>
            </div>
        </div>
      </div>
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-gray-800/20 transition-colors">
                      ...
                      <td className="px-6 py-4 font-bold text-white">
                        {psUnit?.name || "TBD"} <span className="text-xs text-gray-500 font-normal">({psUnit?.type})</span>
                      </td>
                      <td className="px-6 py-4 text-gray-400">
                        {profile?.name || "Sistem"}
                      </td>
                      <td className="px-6 py-4 text-gray-400">{formatRupiah(tx.ps_total || 0)}</td>
                      <td className="px-6 py-4 text-gray-400">{formatRupiah(tx.items_total || 0)}</td>
                      ...
                      <td className="px-6 py-4 text-center">
                         <div className="flex gap-2 justify-center">
                           <button 
                              ...
                              className="text-xs px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white rounded-md transition-colors"
                           >
                             Lihat Struk
                           </button>
                           {userRole === "admin" && (
